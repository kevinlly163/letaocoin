...
static CBigNum bnProofOfStakeLimitTestNet(~uint256(0) >> 12);
...
int64 nChainStartTime = 1503014400;
...
const string strMessageMagic = "LeTaoCoin signed Message:\n";
...
int64 GetProofOfWorkReward(int nHeight, int64 nFees, uint256 prevHash)
{
    ...
    if(nHeight == 1)
      {
        nSubsidy = 210000000 * COIN;	// 1 billion coins, that all pow coins
...
const int YEARLY_BLOCKCOUNT = 525600;	// 365 * 1440*2
int64 GetProofOfStakeReward(int64 nCoinAge, unsigned int nBits, unsigned int nTime, int nHeight)
{
  int64 nRewardCoinYear;
  nRewardCoinYear = MAX_MINT_PROOF_OF_STAKE; //0.5%每年
  ...
  int64 nSubsidy = 0*nCoinAge * nRewardCoinYear / 365;
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in LeTaoCoin
    bool fStrictPayToScriptHash = true; // Always active in LeTaoCoin
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "LeTaoCoin",CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "zaobi.org qq:260682605";
        CTransaction txNew;
        ...
        block.nNonce   = 13608222;

        //// debug print
        if(block.GetHash()!=hashGenesisBlock){
            uint256 powhash=block.GetHash();
            while (powhash > bnProofOfWorkLimit.getuint256()){
                if (++block.nNonce==0) break;
                //            printf("tested nounce %d",block.nNonce);
        ...
        assert(block.hashMerkleRoot == uint256("0f18e94b9638045893254a5c84124d798de58eb61a03cf91c50045d5a37c5c8b"));
...
void BitcoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    while (fGenerateBitcoins || fProofOfStake)
    {
        ...
        while(true)
        {
            ...
            while(true)
            {
                scrypt_1024_1_1_256_sp(BEGIN(pblock->nVersion), BEGIN(result), scratchpad);
                if (result <= hashTarget)
                {
                    // Found a solution
                    printf("LitecoinMiner:\n");
//                    printf("proof-of-work found  \n  powhash: %s  \ntarget: %s\n", result.GetHex(), hashTarget.GetHex());
                    // Found a solution
